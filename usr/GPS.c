...
void GetGpsData(u32 *latitude,u32 *longitude,u8 sos)
{
    ...
    float lat_f,lot_f,lat_out,lot_out;
    u8 lat_int,lot_int;
    ...
    if (Save_Data.isParseData || sos)
	{
		...
		if(Save_Data.isUsefull || sos)
		{
			...
			lat_f=atof(Save_Data.latitude);//经度
            lat_int=(u32)lat_f % 100;           
            lat_out=(u32)lat_f/100+(float)(lat_int+(lat_f-(u32)lat_f))/60;                        
            *latitude=(u32)(lat_out*10000000);
                        
            
            lot_f=atof(Save_Data.longitude);//纬度
            lot_int=(u32)lot_f % 100;
            lot_out=(u32)lot_f/100+(float)(lot_int+(lot_f-(u32)lot_f))/60;                      
            *longitude=(u32)(lot_out*10000000);
