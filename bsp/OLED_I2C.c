...
void OLED_ClearBMP(unsigned char x0,unsigned char y0,unsigned char x1,unsigned char y1)
{
	unsigned int j=0;
	unsigned char x,y;
...
void OLED_ROLL(u32 ms)
{
    if(ms<2)
       ms=2;
    
    bsp_StartTimer(1,ms); //4700
...
void Search_Dsip(void)
{
  //  WriteCmd(0xa7); //--set inverse display 
    for(u8 i=6;i<9;i++)
        OLED_ShowCN(32+(i-6)*16,3,i);//测试显示中文
